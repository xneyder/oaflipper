...
export async function POST(req) {
  ...
  const { productMatchId } = await req.json();
  ...
  try {
    // Update the ProductMatch row with manual_invalid set to false
    const { error: updateError } = await supabase
      .from("ProductMatch")
      .update({ manual_invalid: true }) // Set manual_invalid to true for "No Match"
    ...
    return NextResponse.json({ message: "Product match marked as 'No Match'" }, { status: 200 });
  } catch (error) {
